...
After [profiling a production build](/blogs/how-to-identify-build-time-bottlenecks-using-cpu-profiling) on a large Next.js project with Module Federation, the bottom-up view pointed to `moduleGraph.isDeferred()` inside `ConcatenatedModule._getModulesWithInfo()`. The function was invoked thousands of times per build. Every call returned `false` — but only after walking the module's full incoming connection list.
